...
| `services/` | Бухгалтер | Выполняет операции по правилам |
...
ports/           ← импортирует domain (для типов в интерфейсах)
    ↑
services/        ← импортирует domain и ports
```
...
from core.services import get_problem, get_problem_summaries
...
# Services — используем use cases
problem = get_problem(1, repo, locale="ru")
summaries = get_problem_summaries(repo, difficulty=Difficulty.EASY)
```
